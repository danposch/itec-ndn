...
int main(int argc, char* argv[])
{
  ...
  std::string topologyFile = "topologies/example_tree_network.top";
  ...
  NodeContainer dataStreamersB;
  nodeIndex = 0;
  nodeNamePrefix = std::string("DataStreamerA");
  ...
  nodeIndex = 0;
  nodeNamePrefix = std::string("DataStreamerB");
  dataStreamer = Names::Find<Node>(nodeNamePrefix +  boost::lexical_cast<std::string>(nodeIndex++));
  while(dataStreamer != NULL)
  {
    dataStreamersB.Add (dataStreamer);
    dataStreamer =  Names::Find<Node>(nodeNamePrefix +  boost::lexical_cast<std::string>(nodeIndex++));
  }
  ...
  ndnHelper.Install (dataStreamersB);
  ndnHelper.Install (providers);

  ndnHelper.SetOldContentStore ("ns3::ndn::cs::Fifo","MaxSize", "100000");
  ...
  for(int i=0; i<dataStreamersB.size (); i++)
  {
    if(useRequestAggregation.compare("true") == 0)
      consumerDataHelper.SetPrefix(std::string("/data/B"));
    else
      consumerDataHelper.SetPrefix(std::string("/data/B/"+boost::lexical_cast<std::string>(i)));

    ApplicationContainer consumer = consumerDataHelper.Install (dataStreamersB.Get (i));
    consumer.Start (Seconds(r->GetInteger (0,10)));
    consumer.Stop (Seconds(simTime));

    ns3::ndn::L3RateTracer::Install (dataStreamersB.Get (i), std::string(outputFolder + "/datastreamer-aggregate-trace_"  + boost::lexical_cast<std::string>(i+dataStreamersA.size())).append(".txt"), Seconds (simTime));
    ns3::ndn::AppDelayTracer::Install(dataStreamersB.Get (i),std::string(outputFolder +"/datastreamer-app-delays-trace_"  + boost::lexical_cast<std::string>(i+dataStreamersA.size())).append(".txt"));
  }
