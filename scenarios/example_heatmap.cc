...
int main (int argc, char *argv[])
{
  ...
  else if (strategy.compare ("oracle") == 0)
  {
    ...
    for(int i = 0; i < c.size(); i++)
    {
      ...
      std::string oldname = Names::FindName (c.Get (i));
  ...
  //gen.exportCoreNetworkWithFaceInformation ("/home/dposch/heatmap_top.csv");

  ns3::ndn::L3RateTracer::Install (gen.getAllASNodes (), std::string(outputFolder + "/heat-map-trace.txt"), Seconds(simTime));
